...
void AddSuccCnt()
{
	...
	if (now >iTime)
	{
		printf("time %ld Succ Cnt %d Fail Cnt %d\n", iTime, iSuccCnt, iFailCnt);
		iTime = now;
		iSuccCnt = 0;
		iFailCnt = 0;
	}
	...
	for(;;)
	{
		if ( fd < 0 )
		{
			...
			{       
				...
				if ( ret == -1 ) 
				{       
					printf("getsockopt fd %d ERROR ret %d %d:%s\n", fd, ret, errno, strerror(errno));
					close(fd);
					fd = -1;
					AddFailCnt();
					continue;
        ...
        //printf("send %s from socket fd %d\n", str, fd);
		ret = write( fd,str, 8);
		if ( ret > 0 )
		{
			...
			if ( ret <= 0 )
			{
				printf("co %p read fd %d ret %d errno %d (%s)\n",
						co_self(), fd, ret,errno,strerror(errno));
				close(fd);
				fd = -1;
				AddFailCnt();
			}
