...
int co_set_timeout(int fd, int read_timeout_ms, int write_timeout_ms)
{
    ...
    lp->read_timeout.tv_sec = read_timeout_ms / 1000;
    lp->read_timeout.tv_usec = read_timeout_ms % 1000 * 1000;
    
    lp->write_timeout.tv_sec = write_timeout_ms / 1000;
    lp->write_timeout.tv_usec = write_timeout_ms % 1000 * 1000;
...
int co_pipe( int pfd[2] )
{
    ...
    alloc_by_fd( pfd[0] );
    fcntl( pfd[0], F_SETFL, g_sys_fcntl_func(pfd[0], F_GETFL,0 ) );
    
    //lp1->domain = AF_LOCAL;
    
    //rpchook_t *lp2 = alloc_by_fd( pfd[1] );
    //lp2->domain = AF_LOCAL;
    
    //fcntl( pfd[0], F_SETFL, g_sys_fcntl_func(pfd[0], F_GETFL,0 ) );
    //fcntl( pfd[1], F_SETFL, g_sys_fcntl_func(pfd[1], F_GETFL,0 ) );
    return 0;
}
*/
struct stCoSysEnv_t
