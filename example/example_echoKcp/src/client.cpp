...
void *testRoutine(void *arg) {
    ...
    std::shared_ptr<KcpClient> client(new KcpClient(host, port, local_port, true, true, true, true, crypter));
    ...
    while (true) {
        if (serverReady) {
//DEBUG_LOG("send foo request, tag:%d\n", sendTag+1);
            // send FooRequest
            std::shared_ptr<FooRequest> request(new FooRequest);
            request->set_text("hello world!");
            request->set_times(10);
